...
Defines the local web UI started by the `browse` command: a second interface onto the same backlog store, built for the human loop of skimming what has accumulated, opening a task, editing it and moving to the next one without re-typing an identifier. Every machine-facing path keeps using the existing commands unchanged.
